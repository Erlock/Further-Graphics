...
public class OpenGLApplication {
    ...
    private static double quantum = 0.01;
...
//                y = heightmap[row][col];
//                int index = row * heightmap_width_px + col;
//                vertPositions[3 * index] = x;
//                vertPositions[3 * index + 1] = y;
//                vertPositions[3 * index + 2] = z;
//            }
//        }
...
//                // Adding indices of the lower triangle
...
//
//                // Adding indices of the upper triangle
//                indices[index_count++] = vert_index;
//                indices[index_count++] = vert_index + heightmap_width_px + 1;
...
//        int heightmap_height_px = heightmap.length;
//
//        int num_verts = heightmap_height_px * heightmap_width_px;
...
//                Vector3f vertNormal = new Vector3f(Tz.cross(Tx).normalize());
//                vertNormals[3 * vert_index] = vertNormal.get(0);
//                vertNormals[3 * vert_index + 1] = vertNormal.get(1);
//                vertNormals[3 * vert_index + 2] = vertNormal.get(2);
//            }
//        }
//
//        return vertNormals;

        float[] vertNormals = new float[36 * 3];
        int vertIndex = 0;
        // Left
        for (int i = 0; i < 6; i++) {
            vertNormals[3 * vertIndex] = -1;
            vertNormals[3 * vertIndex + 1] = 0;
            vertNormals[3 * vertIndex + 2] = 0;
        ...
        // Top
        for (int i = 0; i < 6; i++) {
            vertNormals[3 * vertIndex] = 0;
            vertNormals[3 * vertIndex + 1] = 1;
            vertNormals[3 * vertIndex + 2] = 0;
...
//        );
//        int rotPosition = glGetUniformLocation(shaders.getHandle(), "rotationMatrix");
//
//        FloatBuffer rot_buffer = BufferUtils.createFloatBuffer(9);
//
//
...
//        glUniformMatrix3fv(rotPosition, false, rot_buffer);
        ...
        if (camera_loc != -1) {
            Vector3f cam_position = camera.getCameraPosition();
            glUniform3f(camera_loc, cam_position.get(0), cam_position.get(1), cam_position.get(2));
        }
